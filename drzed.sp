...
* Pick a random category that has not yet been picked
  - Enumerate items within that category. If none, pick again (unimplemented category).
  - Items in category n have flags & (1<<n)
  - Category n is described as weapondata_categories[n] + "s" or something similar. TODO: Check it.
* Optionally choose "unique item" mode, if there are at least 3-4 in the category
  - In "unique item" mode, spawn exactly one of one item, and multiples of some others (randomly)
  - otherwise, spawn items randomly, and sum their attributes
* While spawning items, add their attributes to other categories that they're in
* Track quantity in the category, and quantity of each item spawned
* When generating a new challenge, it is acceptable to use "unique item" mode IFF it is still true
